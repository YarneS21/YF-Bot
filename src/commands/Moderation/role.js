    ...
    async execute(interaction) {
        ...
        switch (sub) {
            ...
            if (!guildrole) return await interaction.reply({ content: `You **must** select a **valid** role, **cannot** delete **nothing**..`, ephemeral: true});
                ...
                .setTimestamp()
                .setTitle(`${config.reply} Role Deleted Successfully`)
                .addFields({ name: `• Role ID`, value: `${config.reply} ${guildrole.id}`})
                .addFields({ name: `• Executer`, value: `${config.reply} ${interaction.user}`})
                .setThumbnail(config.picture)
            ...
            .addFields({ name: `• Role`, value: `${config.reply} ${newrole}`})
            .addFields({ name: `• Role Color`, value: `${config.reply} ${color}`})
            .addFields({ name: `• Display Separately`, value: `${config.reply} ${displayside}`, inline: false})
            .addFields({ name: `• Permissions`, value: `${config.reply} ${permissionsarray.join(', ').slice(0, 1000)}`, inline: false})
